...
export default function ProductCard({ product,showDiscount=true }: { product: Product,showDiscount?:boolean }) {
  const discountedPrice = (
    product.price *
    (1 - product.discountPercentage / 100)
  ).toFixed(2);
  return (
    <article className="relative  min-w-56 h-[350px] p-4 rounded-2xl bg-[#f6f6f6] flex flex-col gap-3 ">
      <button
      ...
      <div className="flex flex-col items-center gap-1">
        {product.discountPercentage > 0 && <p className="text-gray-400 line-through text-xs">₹{product.price}</p>}
